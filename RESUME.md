...
## 🎯 الحالة الحالية (21 أغسطس 2026 — ✅ **الموجة 1 مكتملة بالكامل ومتجاوَزة الخطة + مراجعة أمنية سيبرانية منجَزة**)

**⚠️ ملاحظة تصحيح:** القسم أدناه (22 يوليو) كان يعتقد أن "الفقرات الصدرية CT" هي الخطوة القادمة — لكن التحقق المباشر من Supabase (21 أغسطس) أثبت أنها **أُنجزت فعلياً بنفس يوم 22 يوليو** (12 شريحة، معتمدة 11:54 مساءً)، وأن هناك يوماً كاملاً من العمل (23 يوليو) لم يُوثَّق بهذا الملف إطلاقاً.

**✅ الوضع الفعلي المؤكَّد مباشرة من قاعدة البيانات الحيّة (21 أغسطس 2026):**

| العضو | المودالتي | الشرائح | تاريخ الاعتماد | ملاحظة |
|---|---|---|---|---|
| Liver | CT | 4 | 20 يوليو | موثّق سابقاً |
| Brain | CT | 5 | 21 يوليو | موثّق سابقاً |
| Lungs | CT | 5 | 21 يوليو | موثّق سابقاً |
| Lumbar spine | CT | 5 | 22 يوليو | موثّق سابقاً |
| **Thoracic spine** | CT | 12 | 22 يوليو | كانت مذكورة كـ"خطوة قادمة" — **أُنجزت فعلياً بنفس اليوم** |
| **Cervical spine** | CT | 7 | **23 يوليو** | **لم تكن موثّقة بهذا الملف — اكتُشفت بالتحقق المباشر** |
| Chest | X-Ray | 1 | **19 أغسطس** | صورة تجريبية معزولة (مودالتي وحيدة، خارج خطة الموجة 1) — سياقها غير معروف، تحتاج مراجعتك |

**النتيجة:** خطة "الموجة 1 — تفعيل عارض السلاسل" (كانت تستهدف Brain+Liver+Lungs+Lumbar spine) **اكتملت وتجاوزت الخطة** — 6 أعضاء CT بدل 4 مخطَّطة. لا يوجد أي عنصر معلَّق حالياً في `review_queue` (كل شيء إما معتمد أو مرفوض قديم).

**✅ أيضاً — مراجعة أمنية سيبرانية شاملة اكتملت (20-21 أغسطس 2026):** تسريب خطير في 4 دوال Supabase (مفاتيح fal.ai/Gemini/OpenAI/Resend كانت تُرجَع خاماً لأي زائر anon) أُغلق فعلياً ومُتحقَّق منه + إصلاح كشف `auth.users`/طابور المراجعة + تحصين 32 دالة + تنظيف شامل لتوثيق المشروع (تفاصيل كاملة محفوظة في `docs/_archived/OMNIRAD_ISSUES.md` بالمستودع). **إجراء متبقٍّ على المالك:** تدوير المفاتيح الأربعة + تفعيل Leaked Password Protection يدوياً.

**رسالة البداية للمحادثة القادمة:**
```
تابع من هنا — الموجة 2: المقارنة (Comparison) — الدماغ CT+MRI
```

---

## 🎯 حالة سابقة (22 يوليو 2026 — 🧩 **Prompt Skeleton v4 — هيكل البرومت الموحّد منجَز**)
...
- **D14+ فصل لفافي عام + D15 قفل المدى (معتمَد ومُثبَت · بصمة v4c):** بند «فصل لفافي عضوي بين كل التراكيب المجاورة بلا نمط محاك» لكل الأعضاء + `scopeLine()` تمنع خروج المولّد لمناطق مجاورة/خاطئة خارج حقل الرؤية. مُثبَت حيّاً: عمود صدري سهمي + عمود كامل سهمي + كبد CT محوري — كلها واقعية.
...
**رسالة البداية للمحادثة القادمة:**
```
تابع من هنا — رفع الحالات · الموجة 1 / الخطوة 3: الفقرات الصدرية CT (12 مستوى · نافذة Bone) بهيكل Prompt Skeleton v4
```

---

## 🎯 حالة سابقة (22 يوليو 2026 — 🚀 **رفع الحالات — الموجة 1 / الخطوة 2 منجَزة + منتقي LOINC في المراجعة**)

**✅ منجَز اليوم (22 يوليو 2026):**
1. **الموجة 1 / الخطوة 2 — الفقرات القطنية CT سلسلة 5 (نافذة Bone)** — ✅ **معتمدة وحيّة** (تحقّق بصري: 5 شرائح Spine بنفسجي · 100% coherent · شارة قياسية **LOINC 24963-1 · CT Lumbar spine**).
2. **إصلاح تصنيف Spine في شريط التشريح (bug):** `omnirad-anatomy-ribbon.js` كان يصنّف `vertebral body` كـHead/Neck — سبب: الكلمة `vertebral` المجرّدة في regex الخاص بـHEAD_NECK (المقصود `vertebral artery`). صُحّح: HEAD_NECK صار `vertebral artery` فقط + SPINE أُثري بـ`conus|medullar|cauda|thecal|facet|neural foramen`. بصمة `?v=ribbon-spine-fix` على review+atlas.
3. **منتقي LOINC داخل review.html (تصحيح الخيار قبل الاعتماد):** زر **🔧 تعيين الكود القياسي** يظهر بجانب شارة «Non-standard» للسلاسل بلا LOINC → نافذة بحث LOINC/RadLex (منتقي Modality مُملأ مسبقاً + بحث نصّي مزروع باسم العضو + قائمة نتائج) → الاختيار يكتب `loinc_code + radlex_playbook_id + series_technique + naming_tier='standard'` على كل شرائح السلسلة ثم يُعيد التحميل. يعالج جذر «الإدخال الناقص من المستخدم» لكل السلاسل القادمة (نفس صلاحية update المستخدمة لحفظ البيانات التعليمية — لا مشكلة RLS).

**تحقّق:** LOINC للفقرات القطنية موجود فعلاً في قاعدة المنصّة (٧٬٠٨٩ كوداً · ٢٢٣ قطني · ٣٥ CT قطني) → «Non-standard» سببه إدخال ناقص لا نقص في القاعدة. أدقّ كود لسلسلة طبيعية بلا حقن: `30620-9 CT Lumbar spine WO contrast` (استُخدم `24963-1 CT Lumbar spine` العام في هذه الحالة).

**📦 للرفع على GitHub (سُلّمت مفردة بمساراتها 22 يوليو):** `modules/omnirad-anatomy-ribbon.js` · `pages/review.html` · `pages/atlas.html`.

**قاعدة عمل جديدة مثبّتة (CLAUDE.md بند ٤-أ):** الملفات المعدّلة تُسلَّم **مفردة** في المحادثة (بطاقة تحميل لكل ملف) مع ذكر موقع الرفع على المستودع — لا zip مجمّع.

**رسالة البداية للمحادثة القادمة:**
```
تابع من هنا — رفع الحالات · الموجة 1 / الخطوة 3: الدماغ CT سلسلة 5
```
> ملاحظة: الدماغ CT والكبد CT موجودان مسبقاً في القاعدة — الخطوة 3 قد تكون **الفقرات الصدرية CT** لإكمال الموجة 1، أو إثراء variant ثانٍ. يُحدَّد النطاق في بداية المحادثة.

---

## 🎯 حالة سابقة (21 يوليو 2026 — مساءً · 🚀 **بدء رفع الحالات الفعلية — الموجة 1 قيد التنفيذ**)

**بروتوكول العمل المعتمَد (رفع الحالات):**
- التوليد **من الاستوديو على المنصّة فقط** — Claude يعطي **إعدادات الاستوديو** لكل خطوة (لا يُنتج برومبتاً)، المستخدم يولّد عبر ChatGPT ويرفع ويعتمد.
- **«تم» = الصورة وُلّدت + اعتُمدت + ظهرت حيّة في الأطلس** — إشارة إنهاء المهمة والانتقال للتالية. وحدة واحدة (عضو/سلسلة) كل خطوة.

**خطة الموجات (مربوطة بالصفحات):**
- **الموجة 1 — تفعيل عارض السلاسل:** الدماغ CT✅ · الكبد CT✅ (موجودان مسبقاً) · **الرئتان CT ✅ (منجَز اليوم)** · الفقرات القطنية CT (التالي). تُضيء atlas Series/Cine/Ribbon/Coherence.
- **الموجة 2 — المقارنة:** الدماغ CT+MRI · الركبة MRI+X-Ray · الكبد CT+US. تُضيء comparison + المودالتي الديناميكية + dictionary.
- **الموجة 3 — تغطية 9 مناطق:** القلب CT · عظام الحوض CT · العضد X-Ray · الفخذ X-Ray · الثدي Mammography. تُضيء شبكة atlas + عدّاد index.
- **الموجة 4 — طبقة التعلّم (ربط DB لا توليد):** daily · my-progress · mnemonics · clinic · ai-chat.

**قاعدة البيانات الحالية (متحقَّق):** Brain CT (5) · Liver CT (4) · **Lungs CT (5 · LOINC 24627-2 CT Chest)** معتمدة وحيّة.

**✅ منجَز اليوم:**
1. **الموجة 1 / الخطوة 1 — الرئتان CT سلسلة 5** (Apices→Aortic arch→Carina→Hila→Bases · نافذة الرئة · LOINC 24627-2 CT Chest) — معتمدة وظاهرة في الأطلس.
2. **إصلاح الرفض (reject_to_archive):** كان يفشل صامتاً بسبب تعارض overload (bigint/uuid/text). أُعيد تشغيل `reject-to-archive-overload-fix.sql` (يُبقي توقيع text واحد) + أُصلح ابتلاع الخطأ في `review.html doBatchReject` (يُظهر الآن رسالة الخطأ الحقيقية).
3. **إصلاح منتقي LOINC (bulk-upload.html):** كان يسمح باختيار كود عبر-مودالتي (SPECT+CT Lung Views لسلسلة CT). الآن `nmModality` يُزامَن تلقائياً مع مودالتي الصور (`fModality`) → لا يظهر أي كود SPECT/PET/NM لـCT. (`IMG2LOINC` + `syncModalityFromImage` + dispatch عند اختيار سلسلة).
4. **إصلاح شريط التشريح Vascular:** `dominantGroup(structures, regionHint)` صار مدركاً للمنطقة — أوعية المنصف داخل سلسلة صدرية تُحسب Chest لا Vascular. `review.html`+`atlas.html` يمرّران regionHint · بصمة `?v=ribbon-hint`.

**📦 للرفع على GitHub (بمساراتها):** `pages/review.html` · `pages/bulk-upload.html` · `pages/atlas.html` · `modules/omnirad-anatomy-ribbon.js`. (ملف SQL: `reject-to-archive-overload-fix.sql` نُفِّذ في Supabase.)

**نقاط مفتوحة:**
- Vision Check أرجع `0✓` على سلسلة الرئتين (لم يُطابق) — إنذار مبكّر فقط، يُراقَب. تحقّق لاحقاً من استدعاء الدالة/الاستجابة.
- تحقّق بصري بعد الرفع: شريط الرئتين يجب أن يصير **Chest** (أخضر) بدل Vascular.
- **إثراء مؤجَّل — نوافذ CT:** الصدر والفقرات تُقرأ بنافذتين. الرئتان عندها Lung window فقط؛ يُقترح variant ثانٍ **Mediastinal/Soft-tissue** لاحقاً (بعد التحقّق أنّ الأطلس/الرفع يدعم variant ثانٍ لنفس السلسلة). الفقرات القطنية = **Bone window**.

**رسالة البداية للمحادثة القادمة:**
```
تابع من هنا — رفع الحالات · الموجة 1 / الخطوة 2: الفقرات القطنية CT سلسلة 5 (نافذة Bone)
```

---

## 🎯 حالة سابقة (21 يوليو 2026 — ✅ **جولة جودة وتحقّق: P1 + P2 مكتملة**)
...
**متبقٍّ من QA:** لا شيء جوهري — أرشفة `pipeline/` اختيارية (موسومة `_DEPRECATED.md` أصلاً، لا يحمّلها الموقع). جولة QA (P1·P2·P3) مكتملة.
