...
- **متبقٍّ:** مراجعة طبيب أشعة للمستويات/البنى + إثراء ~40 عضواً إضافياً + (اختياري لاحقاً) توسيع vision-check ليُرجع structures[] تلقائياً من الصورة (الخيار الهجين ج).
- **✅ مرحلة توليد السلايس مكتملة ومعتمدة (21 يوليو 2026):** series-slices.js **v3** (+41 عضواً · `?v=series-v3` · معلَّقة لمراجعة طبيب) · مؤشّر 📚 في الاستوديو (قائمة Organ + شارة «سلسلة جاهزة») · إصلاح قسم Series في atlas.html (كان ثابتاً على الدماغ → `refreshAtlasSeriesSection()`) · إصلاح `atlas_related_series()` ليُثبَّت على العضو `p_organ` (كبد↔دماغ خطأ · `?v=related-organ`) · العارض `?v=bc-cine` (W/L→السطوع/التباين · إصلاح زر الإيقاف عبر `updateFrame` في مكانه · تشغيل أبطأ 5FPS نطاق 1-15 · مؤشّر Superior→Inferior سلايدر تفاعلي) · إصلاح `omnirad-term.js` `A.resolveToId is not a function` (`?v=resolveid` على 15 صفحة) · **شارة AI Vision Check في review.html منجَزة** (بطاقة فردية + نقطة مصغّرة + ملخّص سلسلة · إنذار مبكّر فقط · لم تعد مؤجَّلة) · إصلاح صفحة bulk-upload الفارغة (try/catch + forEach مفقود).
- **✅ قسم الجودة والتحقّق — P1+P2 (21 يوليو 2026):** تقرير `docs/QA-AUDIT-2026-07-21.md` · حذف الملف الميّت `modules/series-slices.js` · footer «تعليمي فقط» على 9 صفحات · إصلاح 404 العام (`index-assets/patterns/arabic-geometry.svg` + `index-assets/icons/favicon.svg` مُحقَن من nav.js) · `OmniRadToast` مشترك في nav.js + استبدال كل `alert()` بـtoast ثنائي اللغة (admin/studio/daily). متبقٍّ: P2-7 (توحيد بصمات الكاش) · P3-9 (فحص a11y يدوي).
